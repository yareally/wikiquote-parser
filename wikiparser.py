...
def cmd_line_parse() -> argparse.Namespace:
    ...
    parser.add_argument('--file', dest='filename', metavar='FILE',
                        help='Dump to file instead of stdout. usage [optional]: filename.xml')
...
def format_quote(quote_line: str, quote_id: int, author: str, cats: list) -> Quote:
    ...
    # quote_line =
    # re.sub(r"""[w:]{0,2}           # optionally look for w:
    ...
    return Quote(ident=quote_id, quote=quote_line, author=author, cats=cats)
...
if __name__ == "__main__":
    ...
    save_foreign_title_ref(new_titles=lang_dict,
                           filename='{}/{}.pkl'.format(lang_dir, sanitize_filename(DEFAULT_PAGE)))
    ...
    for lang, page_title in lang_dict.items():
        ...
        if quote_list:
            ...
            dump_xml(xml_data=quote_list, to_file=to_file, langs=lang_dict,
                     filename='{}/{}.xml'.format(default_dir, page_title))
